...
Para verificar quais variáveis possuem uma maior influência no modelo de regressão, vamos aplicar o método de seleção backward, partindo do modelo completo e eliminando uma variável por vez. Porém, considerando fatores que se comportam como uma função linear entre si, teremos como ponto de partida o sequinte modelos:

Modelo 1: **produtividade ~ docentes + produção + tem_doutorado + mestrado + doutorado + mestrado_pprof + doutorado_pprof**

Eliminar variáveis até restarem 3 (propabilidade + p-vvalor).
plotar distribuição 4 variáveis
Se sobrar tempo (plotar distribuição produtividade vs. fatores)
